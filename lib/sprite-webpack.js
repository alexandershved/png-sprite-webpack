...
function getStyles() {
  ...
  function cssTemplate(list) {
    ...
    _.forEach(list, function(item) {
      ...
      var newClassName = item.meta.baseDir + (item.meta.name ? opts.connector : '') + item.meta.name;
    ...
    return templater({
      ...
      spritesheet: {
        ...
        image: path.join(stylePath, opts.spriteName + (opts.baseName ? opts.connector : '') + opts.baseName + "." + opts.format)
      }}, {
        format: 'sprite',
        formatOpts: {
    ...
    opts.info.forEach(function(sprites, key) {
      ...
      var template = templater({
        ...
        spritesheet: {
          width: width,
          height: height,
          image: path.join(stylePath, opts.spriteName + (styleName ? opts.connector : '') + styleName + "." + opts.format)
        }},{
          format: 'sprite',
          formatOpts: {
            'cssClass': opts.prefix,
      ...
      templates.push({
        name: opts.spriteName + (styleName ? opts.connector : '') + styleName,
        template: template,
        cssPath: cssPath
      });
      spritesData = [];
...
function createImagesInfo() {
  ...
  info.forEach(function(sprites, key) {
    ...
    imagesInfo.push({
      info: sprites,
      spriteName: opts.spriteName + (spriteName ? opts.connector : '') + spriteName,
      imgPath: imgPath
    });
  });
  return imagesInfo;
...
module.exports = {
  ...
  createStyles: function(options) {
    ...
    var cssName = opts.spriteName + (opts.baseName ? opts.connector : '') + opts.baseName;
    switch (opts.bundleMode) {
      case opts.oneBundle:
  ...
  createImage: function(options) {
    ...
    function createOne() {
      ...
      opts.spriteName = opts.spriteName + (opts.baseName ? opts.connector : '') + opts.baseName;
      return create(opts);
    }
    switch (opts.bundleMode) {
      case opts.oneBundle:
  ...
  addImport: function(options) {
    ...
    var content = getStyles();
    var spriteIndex = path.join(opts.cssPath, opts.spriteName + (opts.indexName ? opts.connector : '') + opts.indexName + '.' + opts.processor);
    try {
