  ...
  function cssTemplate(list) {
    ...
    return templater({
      ...
      spritesheet: {
        ...
        image: path.join(stylePath, opt.spriteName)
      }}, {
        format: 'sprite',
        formatOpts: {
          'cssClass': opt.prefix,
...
module.exports = {
  ...
  addImport: function(opt) { //use options
    if (!opt.info) { opt.info = updateLayer(opt); }
    if (!opt.useImport) return;
    var content = getStyles(opt); //use options
    var spriteIndex = path.join(opt.cssPath, opt.indexName + '.' + opt.processor)
    try {
    ...
    _.forIn(content, function(v, i) {
      var importPath = path.relative(opt.cssPath, v.cssPath);
      fs.appendFileSync(
        spriteIndex,
